...
describe('FeishuChannel', () => {
  ...
  describe('sendMessage', () => {
    it('sends message via Feishu API as post', async () => {
      const opts = createTestOpts();
      const channel = new FeishuChannel('app_id', 'app_secret', opts);
      await channel.connect();
      ...
      expect(currentClient().im.v1.message.create).toHaveBeenCalledWith({
        ...
        data: {
          ...
          content: expect.stringContaining('"tag":"text"'),
          msg_type: 'post',
        },
      });
    });
    ...
    it('sends even before connect (uses client directly)', async () => {
      const opts = createTestOpts();
      const channel = new FeishuChannel('app_id', 'app_secret', opts);

      // sendMessage uses client directly, not wsClient
      await channel.sendMessage('feishu:oc_test123', 'Before connect');

      expect(currentClient().im.v1.message.create).toHaveBeenCalled();
    });
  });
  ...
  describe('setTyping', () => {
    it('calls typing API when isTyping is true', async () => {
      const opts = createTestOpts();
      const channel = new FeishuChannel('app_id', 'app_secret', opts);
      await channel.connect();

      // Reset request mock after connect (which calls it for bot info)
      currentClient().request.mockClear();
      currentClient().request.mockResolvedValue({});

      await channel.setTyping('feishu:oc_test123', true);

      expect(currentClient().request).toHaveBeenCalledWith({
        method: 'POST',
        url: 'https://open.feishu.cn/open-apis/im/v1/chats/oc_test123/typing',
    ...
    it('does nothing when isTyping is false', async () => {
      ...
      await channel.setTyping('feishu:oc_test123', false);

      expect(currentClient().request).not.toHaveBeenCalled();
    });
  ...
  describe('bot commands', () => {
    it('/chatid replies with chat ID and metadata', async () => {
      ...
      // Should send a reply (as post), not deliver as regular message
      const calls = currentClient().im.v1.message.create.mock.calls;
      const lastCall = calls[calls.length - 1][0];
      expect(lastCall.data.receive_id).toBe('oc_test123');
      expect(lastCall.data.msg_type).toBe('post');
      expect(lastCall.data.content).toContain('feishu:oc_test123');
    ...
    it('/chatid shows chat type', async () => {
      ...
      const calls = currentClient().im.v1.message.create.mock.calls;
      const lastCall = calls[calls.length - 1][0];
      expect(lastCall.data.content).toContain('p2p');
    });
    ...
    it('/ping replies with bot status', async () => {
      ...
      const calls = currentClient().im.v1.message.create.mock.calls;
      const lastCall = calls[calls.length - 1][0];
      expect(lastCall.data.content).toContain('Andy is online');
      expect(opts.onMessage).not.toHaveBeenCalled();
    });
  });
